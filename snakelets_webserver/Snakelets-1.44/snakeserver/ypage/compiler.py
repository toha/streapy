#############################################################################
#
#	$Id: compiler.py,v 1.1 2007/11/25 05:37:53 streapy Exp $
#	Ypage compiler
#
...
class PageCompiler:
    ...
    def processDeclarations(self, syntaxtree):
        ...
        while syntaxtree and ( isinstance(syntaxtree[0],  parser.Declaration) or isinstance(syntaxtree[0], parser.Whitespace) ):
            ...
            if c.name=='import':
                ...
                import sets
                self.authorized=sets.Set(c.value.split(','))
    ...
    def _generateCode(self,c,blockindent):
        ...
        if isinstance(c, parser.Document):
            ...
            if self.authorized:
                code.append(self.indentChars+("def getAuthorizedRoles(self): return sets.Set(%r)" % (tuple(self.authorized), ) ) )
            if self.authmethod:
