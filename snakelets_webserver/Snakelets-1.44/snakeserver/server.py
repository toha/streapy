#############################################################################
#
#	$Id: server.py,v 1.1 2007/11/25 05:37:47 streapy Exp $
#	The HTTP Server
#
...
SNAKELETS_VERSION = "Snakelets 1.44"
...
class ThreadingHTTPServer(object):
    ...
    def __init__(self,HTTPD_PORT, externalPort, bindname, serverURLprefix, debugRequests, precompileYPages, writePageSource):
        ...
        # add the userlibs directory to the module search path
    ...
    def _listAvailableWebapps(self):
    ...
    def loadPluginsAndWebApps(self):
        ...
        if not vhostconfig.ENABLED:
            ...
            webapps = self._listAvailableWebapps()
    ...
    def callSessionCreatedPlugins(self, webapp, session, request):
        if webapp.sharedSession:
            vhost = webapp.getVirtualHost()[0]
            sessionID=session.getID()
            for wa in self.virtualHosts[vhost]:
                if wa.sharedSession:
                    wa.server.plugins.sessionCreated(wa,session,request) # call plugins for sharing webapp
            if vhost in self.webRoots:
                rootapp=self.webRoots[vhost]
                if rootapp.sharedSession:
                    rootapp.server.plugins.sessionCreated(rootapp,session,request) # call plugins for sharing webapp
        else:
            webapp.server.plugins.sessionCreated(webapp,session,request) # call plugins for original webapp

    def callSessionDestroyedPlugins(self, webapp, session, request):
        if webapp.sharedSession:
            vhost = webapp.getVirtualHost()[0]
            sessionID=session.getID()
            for wa in self.virtualHosts[vhost]:
                if wa.sharedSession:
                    wa.server.plugins.sessionDestroyed(wa,session,request) # call plugins for sharing webapp
            if vhost in self.webRoots:
                rootapp=self.webRoots[vhost]
                if rootapp.sharedSession:
                    rootapp.server.plugins.sessionDestroyed(rootapp,session,request) # call plugins for sharing webapp
        else:
            webapp.server.plugins.sessionDestroyed(webapp,session,request) # call plugins for original webapp
    ...
    def listDir(self, filesyspath, urlpath, out):
        def formatDir(name, stats, idx):
            trclass= ("even","odd") [idx%2]
            return '<tr class="%s"><td><a href="%s/">%s</a></td><td>%s</td></tr>\n' % (trclass, urllib.quote(name), name,
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(stats.st_mtime) ) )
        def formatFile(name, stats, idx):
            trclass= ("even","odd") [idx%2]
            kb,rest=divmod(stats.st_size, 1024)
            if rest:
                kb+=1
            return '<tr class="%s"><td><a href="%s">%s</a></td><td>%s</td><td>%d k</td></tr>\n' % (trclass, urllib.quote(name), name,
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(stats.st_mtime) ), kb )
...
-->
        ...
        if dirlist:
            out.write('\n<table><caption>Directories</caption><tr><th class="name">Directory name</th><th>Last modified</th></tr>\n')
            idx=0
            for name in dirlist:
                idx+=1
                stats=os.stat(os.path.join(filesyspath, name))
                out.write( formatDir(name,stats,idx) )
            out.write("</table>\n")
        if filelist:
            out.write('<table><caption>Files</caption><tr><th class="name">File name</th><th>Last modified</th><th>Size</th></tr>\n')
            idx=0
            for name in filelist:
                idx+=1
                stats=os.stat(os.path.join(filesyspath, name))
                out.write( formatFile(name,stats,idx) )
            out.write("</table>\n")
        else:
...
def main(HTTPD_PORT=80, externalPort=None, bindname=None, serverURLprefix='', debugRequests=False,
         precompileYPages=True, writePageSource=False, serverRootDir=None, runAsUser=None, runAsGroup=None):
